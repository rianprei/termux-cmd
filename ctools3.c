...
/* ORIGINAL COMENTADO (codigo NT usa console API indisponivel no shim):
...
#ifndef DOSWIN32
        switch ( fh ) {
        case STDIN:  hFile = GetStdHandle(STD_INPUT_HANDLE);  break;
        case STDOUT: hFile = GetStdHandle(STD_OUTPUT_HANDLE); break;
        case STDERR: hFile = GetStdHandle(STD_ERROR_HANDLE);  break;
        }
#endif
