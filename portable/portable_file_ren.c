...
static RFBUF rfbufs[8];

static RFBUF *rfbuf_get(FILE *h) {
  int i;
  for (i = 0; i < 8; i++) {
    if (rfbufs[i].h == h) return &rfbufs[i];
    if (!rfbufs[i].h) {
      rfbufs[i].h = h;
      rfbufs[i].seekable = (fseek(h, 0, SEEK_CUR) == 0);
      rfbufs[i].len = rfbufs[i].pos = 0;
      return &rfbufs[i];
    }
  }
  return NULL;
...
BOOL WINAPI ReadFile(
  ...
  RFBUF *rb = rfbuf_get(hFile);
  unsigned char *dst = (unsigned char*)lpBuffer;
  size_t want = nNumberOfBytesToRead, done = 0;

  if( !rb ) {
    done = fread(lpBuffer, 1, want, hFile);
  } else if( rb->seekable ) {
    done = fread(lpBuffer, 1, want, hFile);
  } else {
    int saw_nl = 0;
    while( want > 0 && !saw_nl ) {
      if( rb->pos >= rb->len ) {
        ssize_t nr = read(fileno(hFile), rb->buf, sizeof(rb->buf));
        if( nr <= 0 ) break;
        rb->len = (size_t)nr;
        rb->pos = 0;
      }
      ...
      unsigned char *nl = memchr(rb->buf + rb->pos, '\n', take);
      if( nl ) {
        take = (size_t)(nl - (rb->buf + rb->pos)) + 1;
        if( take > want ) {
          take = want;
        } else {
          saw_nl = 1;
...
DWORD WINAPI GetFullPathName(
  ...
  if( unix_path[0] == '/' && unix_path[1] == '\0' ) {
    ...
    sep = strrchr(unix_path, '/');
    dir_part = sep ? strndup(unix_path, sep - unix_path) : strdup(".");
    base_part = sep ? strdup(sep + 1) : strdup(unix_path);
  }

  if( realpath(dir_part, resolved_path) || getcwd(resolved_path, PATH_MAX) != NULL ) {
    if( base_part && *base_part ) {
      strcat(resolved_path, "/");
      strcat(resolved_path, base_part);
    }
  }
  else {
    SetLastErrno();
...
{
	char *unix_path = DOSPath2UNIXPath(lpFileName);
	if (!unix_path) return INVALID_HANDLE_VALUE;
	TCHAR szOpenFlag[8];
	...
	FILE* fp = fopen(unix_path, szOpenFlag);
	free(unix_path);
	if(fp == NULL)
	{
		return INVALID_HANDLE_VALUE;
