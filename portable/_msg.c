...
DWORD WINAPI FormatMessage(
  ...
  _In_opt_ va_list *Arguments
   ...
   BOOLEAN AllocateBuffer = dwFlags & FORMAT_MESSAGE_ALLOCATE_BUFFER;
  ...
  if( msg == NULL ) {
    DWORD len = (nSize > 64) ? nSize : 64;
    LPSTR alloc = malloc(len);
    if( alloc == NULL ) {
      SetLastError(ERROR_NOT_ENOUGH_MEMORY);
      return 0;
    }
    int n = snprintf(alloc, len, "Message: %d (%x)\n", dwMessageId, dwMessageId);
    if( AllocateBuffer ) {
      *(LPTSTR*)lpBuffer = alloc;
      return n;
    }
    DWORD clen = (n < 0) ? 0 : ((DWORD)n < nSize ? (DWORD)n : nSize - 1);
    strncpy(lpBuffer, alloc, clen);
    lpBuffer[clen] = 0;
    free(alloc);
    return clen;
  }
  ...
  DWORD buflen = AllocateBuffer ? (nSize > 1024 ? nSize : 4096) : nSize;
  ...
  DWORD clen = LengthNeeded < nSize ? LengthNeeded : nSize - 1;
  strncpy(lpBuffer, buf, clen);
  lpBuffer[clen] = 0;
  free(buf);
  return clen;
}
