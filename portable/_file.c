...
#define PIPE_BUF_SZ 8192
static char g_pipebuf[PIPE_BUF_SZ];
static size_t g_pipe_off = 0;
...
BOOL WINAPI ReadFile(
  ...
  LPVOID lpOverlapped
	...
	if (nNumberOfBytesToRead == 0) return TRUE;

	// Handles nao-seekable (pipes): entregar no maximo uma linha por
	// chamada. O cmd.exe consome stdin linha a linha e depois tenta dar
	// seek de volta; em pipe isso falha, entao o excedente fica aqui.
	if (fseek(hFile, 0, SEEK_CUR) < 0) {
		if (g_pipe_off >= g_pipe_len) {
			g_pipe_len = fread(g_pipebuf, 1, sizeof(g_pipebuf), hFile);
			g_pipe_off = 0;
			if (g_pipe_len == 0) return FALSE;
		}
		size_t i = g_pipe_off;
		size_t max = (size_t)nNumberOfBytesToRead;
		while (i < g_pipe_len && (i - g_pipe_off) < max) {
			if (g_pipebuf[i] == '\n') { i++; break; }
			i++;
		}
		DWORD got = (DWORD)(i - g_pipe_off);
		if (got > 0) {
			memcpy(lpBuffer, g_pipebuf + g_pipe_off, got);
			g_pipe_off = i;
		}
		if( lpNumberOfBytesRead ) {
