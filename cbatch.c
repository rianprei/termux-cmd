...
int BatLoop(struct batdata *bdat, struct cmdnode *c)
{
...
struct node *n;
{
    ...
    while (tmp) {
        ...
        cmd_printf(Fmt18, TEXT('0')+(long)tmp->rdhndl, tmp->rdop);
...
int FRecurseWork(TCHAR *path, TCHAR *filepart, struct fornode *pForNode, PCPYINFO fsinfo, int i, TCHAR *argtoks);
int FParseWork(struct fornode *pForNode, int i, BOOL bFirstLoop);
int FLoopWork(struct fornode *pForNode, PCPYINFO fsinfo, int i, TCHAR *argtoks, BOOL bFirstLoop);
...
int SFWork(struct node *n, TCHAR **src, int index, BOOL bFirstLoop)
{
